	...
	Creature(), inventory(), varSkills(), varStats(), inventoryAbilities()
{
	...
	houseInbox = new HouseInbox(ITEM_INBOXHOUSE);
	houseInbox->incrementReferenceCounter();

	//MYREWARD
	reward = new RewardChest(ITEM_REWARDCONTAINER);
...
Player::~Player()
{
	...
	for (const auto& it : depotLockerMap) {
		...
		it.second->removeHouseInbox(houseInbox);
		it.second->decrementReferenceCounter();
	}

	inbox->decrementReferenceCounter();
	houseInbox->decrementReferenceCounter();
...
int32_t Player::getPlayerSkillInfo(playerskillinfo_t playerskillinfo)
{
	...
	switch (playerskillinfo) {
	case SKILL_CRITICAL_HIT_CHANCE:
		return item->getCriticalHitChance();

	case SKILL_CRITICAL_HIT_DAMAGE:
		return item->getCriticalExtraDamage();

	case SKILL_LIFE_LEECH_CHANCE:
		return item->getLifeLeechChance();

	case SKILL_LIFE_LEECH_AMOUNT:
		return item->getLifeLeechExtraDamage();

	case SKILL_MANA_LEECH_CHANCE:
		return item->getManaLeechChance();

	case SKILL_MANA_LEECH_AMOUNT:
		return item->getManaLeechExtraDamage();
...
DepotLocker* Player::getDepotLocker(uint32_t depotId)
{
	...
	if (it != depotLockerMap.end()) {
		...
		houseInbox->setParent(it->second);
	...
	depotLocker->internalAddThing(houseInbox);
...
void Player::sendPing()
{
	...
	if ((timeNow - lastPing) >= 5000) {
		...
		if (client) {
			...
			if (getTile()->hasFlag(TILESTATE_NOLOGOUT) || getTile()->hasFlag(TILESTATE_PVPZONE) || getTile()->hasFlag(TILESTATE_NOPVPZONE)) {
				hasLostConnection = false;
			}
		}
	}
...
void Player::onChangeZone(ZoneType_t newZone)
{
	if (newZone == ZONE_PROTECTION) {
	...
	if (newZone != ZONE_PROTECTION && wasMounted) {
		toggleMount(true);
		wasMounted = false;
	}
...
void Player::onSendContainer(const Container* container)
{
	...
	for (const auto& it : openContainers) {
		...
		for (Item* containerItem : container->getItemList()) {
			g_game.startDecay(containerItem);
		}
	}
}
...
void Player::setNextCastTalkTask(SchedulerTask* task)
{
	if (actionTaskEvent != 0) {
		g_scheduler.stopEvent(actionTaskEvent);
		actionTaskEvent = 0;
...
void Player::addExperience(Creature* source, uint64_t exp, bool sendText/* = false*/)
{
	...
	if (prevLevel != level) {
		...
		if (getLevel() == 70) {
			sendTextMessage(MESSAGE_EVENT_ADVANCE, "After level 70 you don't receive more free blessings, you have only one more death, so you need to buy the blesses.");
			client->sendBlessings(0);
		}
	}
...
void Player::postAddNotification(Thing* thing, const Cylinder* oldParent, int32_t index, cylinderlink_t link /*= LINK_OWNER*/)
{
	...
	if (const Item* item = thing->getItem()) {
		...
		if ((getOperatingSystem() == CLIENTOS_FLASH) && requireListUpdate) {
			sendHotkeyInventory();
		}
...
{
	return (OTSYS_TIME() <= (lastLoad + g_config.getNumber(ConfigManager::LOGIN_PROTECTION)));;
}
	...
	if (target) {
		if (party && !Combat::isPlayerCombat(target)) {
			...
			if (tmpMonster && tmpMonster->isHostile()) {
				...
				isPartyFight = true; //PartyUpdate
			}
		}
	}
}

void Player::onTargetCreatureGainHealth(Creature* target, int32_t)
{
	if (target && party) {
