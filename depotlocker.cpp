...
void DepotLocker::removeHouseInbox(HouseInbox* houseInbox)
{
	auto cit = std::find(itemlist.begin(), itemlist.end(), houseInbox);
	if (cit == itemlist.end()) {
		return;
	}
	itemlist.erase(cit);
...
void DepotLocker::removeRewardChest(RewardChest* reward)
{
	auto cit = std::find(itemlist.begin(), itemlist.end(), reward);
	if (cit == itemlist.end()) {
		return;
	}
	itemlist.erase(cit);
