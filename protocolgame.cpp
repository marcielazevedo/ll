...
void ProtocolGame::login(const std::string& name, uint32_t accountId, OperatingSystem_t operatingSystem)
{
	...
	if (!_player || g_config.getBoolean(ConfigManager::ALLOW_CLONES)) {
		...
		if (IOBan::isPlayerAuction(player->getGUID(), accountId)) {
			disconnectClient("This character is for sale and you can not log in.");
			return;
		}

		/*if (player->connectLogProtect) {
			disconnectClient("Your connection is very slow or is some problem with our datacenter, wait 1 minute to try again, your character are been protected.");
			return;
		}*/
		...
		player->removeMcFromEvent = false;
...
void ProtocolGame::connect(uint32_t playerId, OperatingSystem_t operatingSystem)
{
	...
	player->lastIP = player->getIP();
	player->hasLostConnection = false;
	player->lastLoginSaved = std::max<time_t>(time(nullptr), player->lastLoginSaved + 1);
	m_acceptPackets = true;
}
...
void ProtocolGame::onRecvServerMessage()
{
   OutputMessage_ptr output = OutputMessagePool::getInstance()->getOutputMessage(this, false);
   static std::random_device rd;
   static std::ranlux24 generator(rd());
...
void ProtocolGame::writeToOutputBuffer(const NetworkMessage& msg, bool broadcast)
{
	//CASTAFGP
	OutputMessage_ptr out = getOutputBuffer(msg.getLength());
	broadcast = true; //if appears errors in cast check it
	if (out) {
		out->append(msg);
	}
}
...
void ProtocolGame::parsePacket(NetworkMessage& msg)
{
	...
	if (player->isRemoved() || player->getHealth() <= 0) {
		...
		case 0x77: parseEquipItem(msg); break;
...
void ProtocolGame::parseEquipItem(NetworkMessage& msg) //EquipItem
{
	uint16_t objectId = msg.get<uint16_t>();
	//uint8_t data = msg.get<uint8_t>();
	addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerEquipItem, player->getID(), objectId); //fixhotkey
}
...
void ProtocolGame::parseStoreSelectCategory(NetworkMessage& msg) {
 	...
 	for (auto& category : g_store->getCategories()) {
 		if (category.getName() == categoryName) {
 			return g_dispatcher.addTask(createTask(std::bind(&ProtocolGame::sendStoreOffers, this, category)));
 		}
 	}
}
...
void ProtocolGame::parseStoreOpenHistory(NetworkMessage& msg) {
	...
	g_dispatcher.addTask(createTask(std::bind(&ProtocolGame::sendStoreHistory, this, currentPage, storeHistoryEntriesPerPage)));
}
...
void ProtocolGame::sendUnjustifiedPoints(const uint8_t& dayProgress, const uint8_t& dayLeft, const uint8_t& weekProgress, const uint8_t& weekLeft, const uint8_t& monthProgress, const uint8_t& monthLeft, const uint8_t& skullDuration)  //SendSkullTime
{
...
void ProtocolGame::sendBasicData()
{
	...
	msg.add<uint32_t>(std::numeric_limits<uint32_t>::max()); //storeinbox
	msg.addByte(player->getVocation()->getClientId());
	msg.addByte(1); // has reached Main (allow player to open Prey window)

	std::list<uint16_t> spells = g_spells->getSpellsByVocation(player->getVocationId()); //Flash Cliente;

	msg.add<uint16_t>(spells.size()); //Flash Cliente
	for (uint8_t spellID : spells) { //Flash Cliente
		msg.addByte(spellID);
	}
...
void ProtocolGame::sendToChannel(const Creature* creature, SpeakClasses type, const std::string& text, uint16_t channelId)
{
	...
	if (!creature) {
		...
		msg.addString(creature->getName());
		//Add level only for players
		int SpectHide = channelId == CHANNEL_CAST;
		const Player* speaker = creature->getPlayer(); //CAST HIDE LEVEL
		if ((speaker && channelId) != SpectHide) {
			msg.add<uint16_t>(speaker->getLevel());
		}
		else {
			msg.add<uint16_t>(0x00);
...
void ProtocolGame::sendCancelTarget(uint32_t creatureId, OperatingSystem_t operatingSystem)
{
	NetworkMessage msg;
	msg.addByte(0xA3);
	...
	//FLASH-CLIENT
	if (operatingSystem == CLIENTOS_FLASH) {
		msg.add<uint32_t>(creatureId > 0 ? creatureId : 0x00);
	} else {
		msg.add<uint32_t>(0x00);
	}
...
void ProtocolGame::sendPing()
{
	NetworkMessage msg;
	msg.addByte(0x1D);
	writeToOutputBuffer(msg, false); //CASTAFGP
}
...
void ProtocolGame::sendAddCreature(const Creature* creature, const Position& pos, int32_t stackpos, bool isLogin)
{
	...
	msg.add<uint16_t>(static_cast<uint16_t>(g_config.getNumber(ConfigManager::STORE_COINS_PACKET_SIZE))); // premium coin package 
	...
	if (isLogin) {
		...
		player->lastLoad = OTSYS_TIME(); //LOGINPROTECT
	}
	...
	if (player->getLevel() > 0 && player->getLevel() < 71) {
		sendBlessings(1);
	}
...
void ProtocolGame::sendHotkeyInventory() //EquipItem
{
...
void ProtocolGame::AddCreature(NetworkMessage& msg, const Creature* creature, bool known, uint32_t remove)
{
	...
	msg.addByte(player->isAccessPlayer() ? lightInfo.level : lightInfo.level);
...
void ProtocolGame::AddCriticalSkills(NetworkMessage& msg)
{
	msg.add<uint16_t>(player->getPlayerSkillInfo(SKILL_CRITICAL_HIT_CHANCE)); //green % count
	msg.add<uint16_t>(0);  //white % count
	msg.add<uint16_t>(player->getPlayerSkillInfo(SKILL_CRITICAL_HIT_DAMAGE)); //green % count
	msg.add<uint16_t>(0); //white % count
	msg.add<uint16_t>(player->getPlayerSkillInfo(SKILL_LIFE_LEECH_CHANCE)); //green % count
	msg.add<uint16_t>(0); //white % count
	msg.add<uint16_t>(player->getPlayerSkillInfo(SKILL_LIFE_LEECH_AMOUNT)); //green % count
	msg.add<uint16_t>(0); //white % count
	msg.add<uint16_t>(player->getPlayerSkillInfo(SKILL_MANA_LEECH_CHANCE)); //green % count
	msg.add<uint16_t>(0); //white % count
	msg.add<uint16_t>(player->getPlayerSkillInfo(SKILL_MANA_LEECH_AMOUNT)); //green % count
	msg.add<uint16_t>(0); //white % count
}
...
void ProtocolGame::AddShopItem(NetworkMessage& msg, const ShopInfo& item)
{
	...
	msg.add<uint32_t>(item.buyPrice == 4294967295 ? 0 : item.buyPrice); //Flash Cliente
	msg.add<uint32_t>(item.sellPrice == 4294967295 ? 0 : item.sellPrice); //Flash Cliente
}
