...
void ProtocolSpectator::setPlayer(Player* p)
{
	player = p;
	if (p == nullptr) {
		client = nullptr;
...
void ProtocolSpectator::parseSpectatorSay(NetworkMessage& msg)
{
	...
	if (client) {
		if (getSpectatorId()) {
			if (getMessageThreshold() > 4) {
				sendTextMessage(TextMessage(MESSAGE_STATUS_SMALL, "You are sending messages too fast. Try again in a few seconds."));
				pingSpectators();
				return;
			}

			incrementMessageThreshold();
...
void ProtocolSpectator::syncKnownCreatureSets()
{
	...
	if (!tile || !tile->getGround()) {
		dispatchDisconnectClient("A sync error has occured.");
		return;
	}
	...
	for (const auto creatureID : casterKnownCreatures) {
		...
		} else if (operatingSystem <= CLIENTOS_FLASH || operatingSystem <= CLIENTOS_BETA) { // otclient freeze with huge amount of creature add, but do not debug if there are unknown creatures, best solution for now :(
...
void ProtocolSpectator::pingSpectators()
{
	if (client) {
		if (getSpectatorId()) {
			if ((OTSYS_TIME() - lastCastSpeak) > 4000) {
				resetMessageThreshold();
				lastCastSpeak = OTSYS_TIME();
			}
		}
	}
}