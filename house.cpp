...
bool House::transferToDepot(Player* player) const
{
	...
	for (Item* item : moveItemList) {
		g_game.internalMoveItem(item->getParent(), player->getHouseInbox(), INDEX_WHEREEVER, item, item->getItemCount(), nullptr, FLAG_NOLIMIT);
	}
	return true;
}
	...
	for (const auto& it : houseMap) {
		...
		if (player.getBankBalance() >= rent) {
			...
			if (house->getPayRentWarnings() < 7) {
				...
				g_game.internalAddItem(player.getHouseInbox(), letter, INDEX_WHEREEVER, FLAG_NOLIMIT);
