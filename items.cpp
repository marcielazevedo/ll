...
void Items::parseItemNode(const pugi::xml_node& itemNode, uint16_t id)
{
	...
	for (auto attributeNode : itemNode.children()) {
		...
		if (tmpStrValue == "type") {
		...
		} else if (tmpStrValue == "especialtile") {
			it.especialTileHouse = valueAttribute.as_bool();
			...
			it.criticalHitChance = std::min<int8_t>(100, std::max<int8_t>(-100, pugi::cast<int16_t>(valueAttribute.value())));
		} else if (tmpStrValue == "criticalextradamage") {
			it.criticalExtraDamage = std::min<int8_t>(100, std::max<int8_t>(-100, pugi::cast<int16_t>(valueAttribute.value())));
		} else if (tmpStrValue == "lifeleechchance") {
			it.lifeLeechChance = std::min<int8_t>(100, std::max<int8_t>(-100, pugi::cast<int16_t>(valueAttribute.value())));
		} else if (tmpStrValue == "lifeleechextradamage") {
			it.lifeLeechExtraDamage = std::min<int8_t>(100, std::max<int8_t>(-100, pugi::cast<int16_t>(valueAttribute.value())));
		} else if (tmpStrValue == "manaleechchance") {
			it.manaLeechChance = std::min<int8_t>(100, std::max<int8_t>(-100, pugi::cast<int16_t>(valueAttribute.value())));
		} else if (tmpStrValue == "manaleechextradamage") {
			it.manaLeechExtraDamage = std::min<int8_t>(100, std::max<int8_t>(-100, pugi::cast<int16_t>(valueAttribute.value())));
