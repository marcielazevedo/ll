...
ReturnValue RewardChest::queryAdd(int32_t, const Thing& thing, uint32_t,
...
void RewardChest::postAddNotification(Thing* thing, const Cylinder* oldParent, int32_t index, cylinderlink_t)
{
	...
	if (Container* container = thing->getContainer()) {
		if (RewardChest* rewardContainer = container->getRewardChest()) {
			rewardContainer->setParentRewardChest(this);
		}
	}
}
...
void RewardChest::postRemoveNotification(Thing* thing, const Cylinder* newParent, int32_t index, cylinderlink_t)
{
	...
	if (Container* container = thing->getContainer()) {
		if (RewardChest* rewardContainer = container->getRewardChest()) {
			rewardContainer->setParentRewardChest(nullptr);
		}
	}
}