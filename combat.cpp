...
ReturnValue Combat::canDoCombat(Creature* caster, Tile* tile, bool aggressive)
{
	...
	//LOGINPROTECT
	if (caster->getPlayer() && caster->getPlayer()->checkLoginDelay()) {
		return RETURNVALUE_YOUMAYNOTATTACKIMMEDIATELYAFTERLOGGINGIN;
	}

	//LOSTCONNECTION
	if (caster->getPlayer() && caster->getPlayer()->hasLostConnection && !caster->getPlayer()->isPzLocked()) {
		return RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE;
	}
...
bool Combat::isProtected(const Player* attacker, const Player* target)
{
	...
	//IF YOU WANT TO PLAYER LOSE LIFE ON ATK(pvp mode off) CHANGE THIS
...
ReturnValue Combat::canDoCombat(Creature* attacker, Creature* target)
{
	if (attacker) {
		if (const Player* targetPlayer = target->getPlayer()) {
			...
			//LOGINPROTECT
			if (attacker->getPlayer() && targetPlayer->checkLoginDelay() && !target->hasBeenAttacked(target->getID())) {
				return RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE;
			}

			//LOGINPROTECT
			if (attacker->getMonster() && targetPlayer->checkLoginDelay() && !target->hasBeenAttacked(target->getID())) {
				return RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE;
			}

			//LOSTCONNECTION
			if (attacker->getMonster() && targetPlayer->hasLostConnection && !targetPlayer->isPzLocked()) {
				return RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE;
			}
...
void Combat::CombatHealthFunc(Creature* caster, Creature* target, const CombatParams& params, void* data)
{
	...
	if ((damage.primary.value < 0 || damage.secondary.value < 0) && caster) {
		...
		if (targetPlayer && caster->getPlayer() && targetPlayer->getSkull() != SKULL_BLACK) {
			...
			//Combat diference among monsters and players
...
void Combat::CombatManaFunc(Creature* caster, Creature* target, const CombatParams& params, void* data)
{
	...
	if (g_game.combatChangeMana(caster, target, damage.primary.value, damage.origin, damage)) {
		CombatConditionFunc(caster, target, params, nullptr);
		CombatDispelFunc(caster, target, params, nullptr);
	}
}
