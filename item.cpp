...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		...
		case ATTR_CRITICALHITCHANCE: {
			int8_t criticalHitChance;
			if (!propStream.read<int8_t>(criticalHitChance)) {
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_CRITICALEXTRADAMAGE: {
			int8_t criticalExtraDamage;
			if (!propStream.read<int8_t>(criticalExtraDamage)) {
				return ATTR_READ_ERROR;
			}

			setIntAttr(ITEM_ATTRIBUTE_CRITICALEXTRADAMAGE, criticalExtraDamage);
			break;
		}

		case ATTR_LIFELEECHCHANCE: {
			int8_t lifeLeechChance;
			if (!propStream.read<int8_t>(lifeLeechChance)) {
				return ATTR_READ_ERROR;
			}

			setIntAttr(ITEM_ATTRIBUTE_LIFELEECHCHANCE, lifeLeechChance);
			break;
		}

		case ATTR_LIFELEECHEXTRADAMAGE: {
			int8_t lifeLeechExtraDamage;
			if (!propStream.read<int8_t>(lifeLeechExtraDamage)) {
				return ATTR_READ_ERROR;
			}

			setIntAttr(ITEM_ATTRIBUTE_LIFELEECHEXTRADAMAGE, lifeLeechExtraDamage);
			break;
		}


		case ATTR_MANALEECHCHANCE: {
			int8_t manaLeechChance;
			if (!propStream.read<int8_t>(manaLeechChance)) {
				return ATTR_READ_ERROR;
			}

			setIntAttr(ITEM_ATTRIBUTE_MANALEECHCHANCE, manaLeechChance);
			break;
		}

		case ATTR_MANALEECHEXTRADAMAGE: {
			int8_t manaLeechExtraDamage;
			if (!propStream.read<int8_t>(manaLeechExtraDamage)) {
				return ATTR_READ_ERROR;
			}

			setIntAttr(ITEM_ATTRIBUTE_MANALEECHEXTRADAMAGE, manaLeechExtraDamage);
			break;
		}
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	if (hasAttribute(ITEM_ATTRIBUTE_CRITICALHITCHANCE)) {
		...
		propWriteStream.write<int8_t>(getIntAttr(ITEM_ATTRIBUTE_CRITICALHITCHANCE));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_CRITICALEXTRADAMAGE)) {
		propWriteStream.write<uint8_t>(ATTR_CRITICALEXTRADAMAGE);
		propWriteStream.write<int8_t>(getIntAttr(ITEM_ATTRIBUTE_CRITICALEXTRADAMAGE));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_LIFELEECHCHANCE)) {
		propWriteStream.write<uint8_t>(ATTR_LIFELEECHCHANCE);
		propWriteStream.write<int8_t>(getIntAttr(ITEM_ATTRIBUTE_LIFELEECHCHANCE));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_LIFELEECHEXTRADAMAGE)) {
		propWriteStream.write<uint8_t>(ATTR_LIFELEECHEXTRADAMAGE);
		propWriteStream.write<int8_t>(getIntAttr(ITEM_ATTRIBUTE_LIFELEECHEXTRADAMAGE));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_MANALEECHCHANCE)) {
		propWriteStream.write<uint8_t>(ATTR_MANALEECHCHANCE);
		propWriteStream.write<int8_t>(getIntAttr(ITEM_ATTRIBUTE_MANALEECHCHANCE));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_MANALEECHEXTRADAMAGE)) {
		propWriteStream.write<uint8_t>(ATTR_MANALEECHEXTRADAMAGE);
		propWriteStream.write<int8_t>(getIntAttr(ITEM_ATTRIBUTE_MANALEECHEXTRADAMAGE));
	}
}
                                 ...
                                 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
	...
	if (it.isRune()) {
		...
		if (it.weaponType == WEAPON_DISTANCE && it.ammoType != AMMO_NONE) {
			...
			//CRITICALSYSTEM
			int32_t attack;
			int8_t hitChance, criticalHitChance, criticalExtraDamage, lifeLeechChance, lifeLeechExtraDamage, manaLeechChance, manaLeechExtraDamage;
			if (item) {
				attack = item->getAttack();
				hitChance = item->getHitChance();
				criticalHitChance = item->getCriticalHitChance();
				criticalExtraDamage = item->getCriticalExtraDamage();
				lifeLeechChance = item->getLifeLeechChance();
				lifeLeechExtraDamage = item->getLifeLeechExtraDamage();
				manaLeechChance = item->getManaLeechChance();
				manaLeechExtraDamage = item->getManaLeechExtraDamage();
			} else {
				attack = it.attack;
				hitChance = it.hitChance;
				criticalHitChance = it.criticalHitChance;
				criticalExtraDamage = it.criticalExtraDamage;
				lifeLeechChance = it.lifeLeechChance;
				lifeLeechExtraDamage = it.lifeLeechExtraDamage;
				manaLeechChance = it.manaLeechChance;
				manaLeechExtraDamage = it.manaLeechExtraDamage;
			}
			...
			if (it.weaponType != WEAPON_WAND && it.weaponType == WEAPON_DISTANCE) {
				if (criticalHitChance != 0) {
					s << ", critical hit chance " << std::showpos << static_cast<int16_t>(criticalHitChance) << std::noshowpos << "%";
				}

				if (criticalExtraDamage != 0) {
					s << ", critical extra damage " << std::showpos << static_cast<int16_t>(criticalExtraDamage) << std::noshowpos << "%";
				}

				if (lifeLeechChance != 0) {
					s << ", hit points leech chance " << std::showpos << static_cast<int16_t>(lifeLeechChance) << std::noshowpos << "%";
				}

				if (lifeLeechExtraDamage != 0) {
					s << ", hit points leech amount " << std::showpos << static_cast<int16_t>(lifeLeechExtraDamage) << std::noshowpos << "%";
				}

				if (manaLeechChance != 0) {
					s << ", mana points leech chance " << std::showpos << static_cast<int16_t>(manaLeechChance) << std::noshowpos << "%";
				}

				if (manaLeechExtraDamage != 0) {
					s << ", mana points leech amount " << std::showpos << static_cast<int16_t>(manaLeechExtraDamage) << std::noshowpos << "%";
				}
			}
			s << ')';
			...
			int8_t criticalHitChance, criticalExtraDamage, lifeLeechChance, lifeLeechExtraDamage, manaLeechChance, manaLeechExtraDamage;
			...
			if (!it.abilities && it.weaponType != WEAPON_WAND) {
				if (criticalHitChance != 0) {
					s << ", critical hit chance " << std::showpos << static_cast<int16_t>(criticalHitChance) << std::noshowpos << "%";
				}
			...
			} else if (it.abilities && it.weaponType != WEAPON_WAND) {
				if (criticalHitChance != 0) {
					s << ", critical hit chance " << std::showpos << static_cast<int16_t>(criticalHitChance) << std::noshowpos << "%";
				}
			...
			if (it.abilities) {
				...
				if (!it.abilities->stats[STAT_MAGICPOINTS] && it.weaponType == WEAPON_WAND) {
					if (criticalHitChance != 0) {
						s << " (critical hit chance " << std::showpos << static_cast<int16_t>(criticalHitChance) << std::noshowpos << "%";
					}
				...
				} else if (it.abilities->stats[STAT_MAGICPOINTS] && it.weaponType == WEAPON_WAND) {
					if (criticalHitChance != 0) {
						s << ", critical hit chance " << std::showpos << static_cast<int16_t>(criticalHitChance) << std::noshowpos << "%";
					}
