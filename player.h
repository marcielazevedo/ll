...
class Player final : public Creature, public Cylinder
{
	public:
		...
		//HOUSEINBOX
		HouseInbox* getHouseInbox() const {
			return houseInbox;
		}
		...
		int32_t getPlayerSkillInfo(playerskillinfo_t playerskillinfo);
		...
		uint32_t getFreeCapacity() const {
			if (hasFlag(PlayerFlag_CannotPickupItem)) {
				...
				if (operatingSystem == CLIENTOS_FLASH || operatingSystem == CLIENTOS_BETA) {
					return 5000000;
				} else {
					return std::numeric_limits<uint32_t>::max();
				}
		...
		void onChangeZone(ZoneType_t newZone) final;
		...
		//EquipItem
		void sendHotkeyInventory() {
			if (client) {
				client->sendHotkeyInventory();
			}
		}
		...
		bool removeMcFromEvent;

		//bool connectLogProtect;
	...
	protected:
		...
		void setNextCastTalkTask(SchedulerTask* task);
		...
		std::map<uint16_t, uint16_t> getAllItemsClientId() const; //EquipItem
		Item* getItemByClientId(uint16_t clientId) const; //EquipItem
		...
		std::map<uint32_t, DepotChest*> depotBoxs;
		...
		Group* group;
		Inbox* inbox;
		HouseInbox* houseInbox;
		RewardChest* reward; //MYREWARD
		Item* tradeItem;
		...
		uint32_t editListId;
		uint32_t maxInboxItems; //MY
		uint32_t maxHouseInboxItems; //MY
		uint32_t manaMax;
