...
bool Commands::exeCommand(Player& player, const std::string& cmd)
{
	...
	if (command->groupId > player.getGroup()->id || command->accountType > player.getAccountType()) {
		if (player.getGroup()->access) {
			if (player.getOperatingSystem() == CLIENTOS_FLASH) {
				player.sendTextMessage(MESSAGE_STATUS_SMALL, "You can not execute this command.");
			} else {
				player.sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "You can not execute this command.");
			}
		}
	...
	if (command->logged) {
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, cmd);
		} else {
			player.sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, cmd);
		}
...
void Commands::reloadInfo(Player& player, const std::string& param)
{
	...
	if (tmpParam == "action" || tmpParam == "actions") {
		g_actions->reload();
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded actions.");
		} else {
		...
		g_config.reload();
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded config.");
		} else {
		...
		reload();
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded commands.");
		} else {
		...
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded creature scripts.");
		} else {
		...
		g_monsters.reload();
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded monsters.");
		} else {
		...
		g_moveEvents->reload();
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded movements.");
		} else {
		...
		Npcs::reload();
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded npcs.");
		} else {
		...
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded raids.");
		} else {
		...
		g_monsters.reload();
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded spells.");
		} else {
		...
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded talk actions.");
		} else {
		...
		Item::items.reload();
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded items.");
		} else {
		...
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded weapons.");
		} else {
		...
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded quests.");
		} else {
		...
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded mounts.");
		} else {
		...
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded globalevents.");
		} else {
		...
		g_events->load();
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded events.");
		} else {
		...
		g_store->reload();
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded store.");
		} else {
		...
		g_chat->load();
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded chatchannels.");
		} else {
		...
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reloaded global.lua.");
		} else {
			player.sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Reloaded global.lua.");
		}
	} else {
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Reload type not found.");
		} else {
...
void Commands::forceRaid(Player& player, const std::string& param)
{
	...
	if (!raid || !raid->isLoaded()) {
		...
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "No such raid exists.");
		} else {
	...
	if (g_game.raids.getRunning()) {
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Another raid is already being executed.");
		} else {
	...
	if (!event) {
		if (player.getOperatingSystem() == CLIENTOS_FLASH) {
			player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "The raid does not contain any data.");
		} else {
	...
	if (player.getOperatingSystem() == CLIENTOS_FLASH) {
		player.sendTextMessage(MESSAGE_EVENT_DEFAULT, "Raid started.");
	} else {
...
//CASTAFGP
void Commands::startCast(Player& player, const std::string& password)
{
	if (!player.isLiveCaster()) {
		player.startLiveCast(password);
