...
bool Monsters::loadMonster(const std::string& file, const std::string& monsterName, std::list<std::pair<MonsterType*, std::string>>& monsterScriptList, bool reloading /*= false*/)
{
	...
	//WALKFIELD DAQUI ATE
	if ((node = monsterNode.child("walkablefields"))) {
		for (auto walkableNode : node.children()) {
			if ((attr = walkableNode.attribute("name"))) {
				std::string tmpStrValue = asLowerCaseString(attr.as_string());
				if (tmpStrValue == "energy") {
					mType->walkableField |= COMBAT_ENERGYDAMAGE;
				}
				else if (tmpStrValue == "fire") {
					mType->walkableField |= COMBAT_FIREDAMAGE;
				}
				else if (tmpStrValue == "poison" || tmpStrValue == "earth") {
					mType->walkableField |= COMBAT_EARTHDAMAGE;
				}
				else {
					std::cout << "[Warning - Monsters::loadMonster] Unknown walkable field " << attr.as_string() << ". " << file << std::endl;
				}
			}
			else if ((attr = walkableNode.attribute("energy"))) {
				if (attr.as_bool()) {
					mType->walkableField |= COMBAT_ENERGYDAMAGE;
				}
			}
			else if ((attr = walkableNode.attribute("fire"))) {
				if (attr.as_bool()) {
					mType->walkableField |= COMBAT_FIREDAMAGE;
				}
			}
			else if ((attr = walkableNode.attribute("poison")) || (attr = walkableNode.attribute("earth"))) {
				if (attr.as_bool()) {
					mType->walkableField |= COMBAT_EARTHDAMAGE;
				}
			}
			else {
				std::cout << "[Warning - Monsters::loadMonster] Unknown walkable field. " << file << std::endl;
			}
		}
	}
	//WALKFIELD ATE AQUI
