...
bool IOMapSerialize::loadItem(PropStream& propStream, Cylinder* parent)
{
	...
	if (iType.moveable || !tile || iType.especialTileHouse) {
		//create a new item
...
void IOMapSerialize::saveTile(PropWriteStream& stream, const Tile* tile)
{
	...
	for (Item* item : *tileItems) {
		...
		if (!(it.moveable || item->getDoor() || (item->getContainer() && !item->getContainer()->empty()) || it.canWriteText || item->getBed() || it.especialTileHouse)) {
			continue;
		}
