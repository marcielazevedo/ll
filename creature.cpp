...
void Creature::gainMana(Creature* healer, int32_t manaGain)
{
	changeMana(manaGain);
	if (healer) {
		healer->onTargetCreatureGainHealth(this, manaGain);
	}
}
...
//WALKFIELD
bool Creature::getWalkableField(CombatType_t type) const
{
	return hasBitSet(static_cast<uint32_t>(type), IsWalkableField());
}
