...
bool ConfigManager::load()
{
	...
	boolean[ENABLE_LIVE_CASTING] = getGlobalBoolean(L, "enableLiveCasting", true); //CASTAFGP
	boolean[FORCE_CLOSE_SLOW_CONNECTION] = getGlobalBoolean(L, "forceSlowConnectionsToDisconnect", false); //LOSTCONNECT
	...
	string[ADVERTISING_BLOCK] = getGlobalString(L, "advertisingBlock", "");
	...
	integer[SPAWN_STAGES] = getGlobalNumber(L, "spawnStages", 1);
	...
	integer[LIVE_CAST_PORT] = getGlobalNumber(L, "liveCastPort", 7173); //CASTAFGP
	...
	integer[LOGIN_PROTECTION] = getGlobalNumber(L, "loginProtectionPeriod", 10000);
	integer[STORE_COINS_PACKET_SIZE] = getGlobalNumber(L, "storeCoinsPacketSize", 25);
	integer[STORE_TIME_TO_NEW] = getGlobalNumber(L, "storeTimeToNew", 2);
	integer[MAX_TILE_ITEMS] = getGlobalNumber(L, "maxTileItems", 90);
	integer[MAX_CAP_ITEMS] = getGlobalNumber(L, "maxCapItems", 2000000);
